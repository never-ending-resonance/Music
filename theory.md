...
<body>
    ...
    // SCALE (and custom scale)
        ...
        try {
            ...
            if (isCustomScale) {
                ...
                // Chord or arpeggio
                // Add more formatting if desired (e.g. "min7b5" => "m7♭5", etc)
        ...
        try {
            ...
            for (let s = 5; s >= 0; s--) {
                ...
                for (let f = 0; f <= fretMax; f++) {
                    ...
                    html += `<div class="${isScaleNote ? 'note-dot' : 'note-empty'}">${isScaleNote ? '●' : '·'}</div>`;
                }
                html += `</div>`;
            }
    ...
    function showThreeDiagrams(root, scaleType, scaleNotes) {
        ...
        try {
            ...
            const diagrams = [
                { label: "Left", startPos: 5, maxNotes: [1, 3, 3, 3] },   // [E, A, D, G] max notes — row 0 = E, row 3 = G
